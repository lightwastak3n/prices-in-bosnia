...
while i < len(urls):
    ...
    try:
        ...
        new_items, items_inserted = konzum_scraper.add_items_to_database(
            server, "konzum"
        )
        total_new += new_items
        total_inserted += items_inserted
        print(
            f"Scraped {url} {category} and found {new_items} new and {items_inserted} total items."
        )
