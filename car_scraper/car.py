...
class Car:
    ...
    def clean_dict_keys(self):
        ...
        for key in self.data:
            if key in specs_columns_mapping and self.data[key] != None:
    ...
    def fix_basic_data(self):
        ...
        if "velicina_felgi" in self.data and (
            self.data["velicina_felgi"] == "Ostalo" or self.data["velicina_felgi"] == ""
        ):
    ...
    def fix_year(self):
        ...
        """
        if (
            "godiste" not in self.data
            or self.data["godiste"] == "Starije"
            or int(self.data["godiste"]) < 1950
        ):
            self.data["godiste"] = 0
    ...
    def fix_first_registration(self):
        ...
        """
        if (
            "prva_registracija" in self.data
            and self.data["prva_registracija"] == "Starije"
        ):
            del self.data["prva_registracija"]
        if (
            "prethodnih_vlasnika" in self.data
            and self.data["prethodnih_vlasnika"] == "Ostalo"
        ):
