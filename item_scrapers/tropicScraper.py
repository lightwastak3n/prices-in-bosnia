        ...
        for item_type in self.htmls:
            ...
            for html in self.htmls[item_type]:
                ...
                for item in links:
                    ...
                    name = (
                        item.find("h2", class_=self.prop_classes["name"])
                        .text.strip()
                        .lower()
                    )
                    price = (
                        item.find("span", class_=self.prop_classes["price"])
                        .find("bdi")
                        .text.strip("KM")
                    )
                    if item.find("span", class_=self.prop_classes["unit"]) is None:
                        ...
                        unit = (
                            item.find("span", class_=self.prop_classes["unit"])
                            .text.strip("/")
                            .lower()
                        )
