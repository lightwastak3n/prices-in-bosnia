...
def catch_all():
    while True:
        try:
            scraper.get_real_estates_from_main()
            new_found = scraper.filter_new_real_estates()
            ...
            for item in not_scraped[:possible_fit]:
                ...
                data = scraper.scrape_real_estate(rs_id, rs_link)
                if data:
                    try:
                        new_rs = RealEstate(data, type)
                    except Exception as e:
                        print(f"Some error occured {e}. Skipping this listing.")
                        write_log_error(f"Some error occured {e}. Skipping this listing.")
                    else:    
...
def catch_big():
    while True:
        try:
            ...
            pause_between_items = randint(30,50)
            possible_fit = min(allocated_time // pause_between_items, len(not_scraped))
            time_left = max(allocated_time - total_not_scraped * pause_between_items, 0)
